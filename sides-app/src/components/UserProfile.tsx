...
import { User, Award, TrendingUp, AlertCircle, CheckCircle2, Shield, Calendar, MapPin } from 'lucide-react';
...
const UserProfile = () => {
  return (
    <div className="min-h-screen bg-[#0F172A] text-slate-300 font-sans">
      ...
      {/* Üst Banner (Kapak) */}
      <div className="h-48 bg-gradient-to-r from-blue-900 to-slate-900 border-b border-slate-800 relative">
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-20 relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-end gap-6 mb-12">
          ...
          {/* Avatar */}
          <div className="w-32 h-32 bg-[#1E293B] rounded-2xl border-4 border-[#0F172A] flex items-center justify-center shadow-2xl">
          ...
          <div className="flex-1 pb-2">
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-3xl font-bold text-white">LogicSeeker_99</h1>
            ...
            <p className="text-slate-400 text-sm flex items-center gap-4">
              ...
              <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> Üyelik: Aralık 2025</span>
            </p>
          </div>

          {/* Ana Skor (İtimat) */}
          <div className="bg-[#1E293B] p-4 rounded-xl border border-slate-700 shadow-lg text-center min-w-[150px]">
            <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">İtimat Skoru</span>
            <div className="text-4xl font-bold text-emerald-400 mt-1">720</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12">
          ...
          <StatCard 
            ...
            label="Kazanılan Argüman"
            value="14"
            desc="Mantıksal üstünlük"
          />
          ...
          <StatCard 
            ...
            label="Teslimiyet"
            value="3"
            desc="Doğruyu kabul etme"
            highlight // Bu kartı öne çıkar
          />

          <StatCard 
            icon={<AlertCircle className="w-5 h-5 text-rose-400" />}
            label="Safsata Uyarısı"
            value="2"
            desc="Son 30 günde"
          />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Sol Kolon: Aktivite Akışı */}
          <div className="lg:col-span-2 space-y-6">
            <h3 className="text-xl font-bold text-white mb-4">Son Aktiviteler</h3>
            
            <ActivityItem 
              type="win"
              title="Yapay Zeka Telif Hakları Tartışması"
              desc="Argümanınız 'Saman Adam' testini başarıyla geçti ve ana ağaca eklendi."
              time="2 saat önce"
            />

            <ActivityItem 
              type="surrender"
              title="Evrensel Temel Gelir"
              desc="Karşı tarafın sunduğu veriyi kabul ederek 'Teslimiyet' puanı kazandınız."
              time="1 gün önce"
            />

             <ActivityItem 
              type="post"
              title="Toplu Taşımada Nezaket"
              desc="Yeni bir antitez oluşturdunuz: 'Ses kirliliği özgürlük değildir'."
              time="3 gün önce"
            />

          </div>

          {/* Sağ Kolon: Rozetler ve Yetenekler */}
...
const StatCard = ({ icon, label, value, desc, highlight }: any) => (
  <div className={`p-5 rounded-xl border flex flex-col justify-between h-32 transition-all hover:scale-[1.02]
    ${highlight 
      ? 'bg-blue-900/10 border-blue-500/50 shadow-[0_0_20px_rgba(59,130,246,0.15)]' 
      : 'bg-[#1E293B] border-slate-700/50'}`}>
    <div className="flex justify-between items-start">
      <div className="p-2 bg-[#0F172A] rounded-lg border border-slate-700">{icon}</div>
      <span className="text-2xl font-bold text-white">{value}</span>
    </div>
    <div>
      <div className={`text-sm font-semibold ${highlight ? 'text-blue-200' : 'text-slate-300'}`}>{label}</div>
...
const ActivityItem = ({ type, title, desc, time }: any) => {
  ...
  let icon = <CheckCircle2 className="w-5 h-5 text-slate-500" />;
  ...
  return (
    <div className={`p-5 rounded-xl bg-[#1E293B] border ${borderColor} relative group overflow-hidden`}>
      ...
      <div className="flex gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1 mb-2">{desc}</p>
...
const Badge = ({ color, name, opacity }: any) => (
  <div className={`aspect-square rounded-xl flex flex-col items-center justify-center gap-2 p-2 
    ${opacity ? 'opacity-30 grayscale' : 'bg-slate-800 border border-slate-700'}`}>
    <div className={`w-8 h-8 rounded-full ${color} shadow-lg`}></div>
    <span className="text-[10px] font-bold text-slate-400 uppercase">{name}</span>
  </div>
);

const SkillBar = ({ name, percent }: any) => (
