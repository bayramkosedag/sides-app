  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-slate-300 font-sans pb-20 md:pb-0">
      ...
      <div className="h-40 md:h-48 bg-gradient-to-r from-blue-900 to-slate-900 border-b border-slate-800 relative">
        ...
        {/* MOBİL İÇİN SAĞ ÜST AYARLAR (Sadece mobilde görünür) */}
        <div className="absolute top-4 right-4 flex gap-2 md:hidden">
            <button className="p-2 bg-slate-800/50 backdrop-blur text-white rounded-full border border-slate-700">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 -mt-16 md:-mt-20 relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-end gap-4 md:gap-6 mb-8 md:mb-12 text-center md:text-left">
          ...
          <div className="flex-1 pb-2 w-full md:w-auto">
            ...
            {/* MASAÜSTÜ İÇİN BUTONLAR (Sadece masaüstünde görünür) */}
...
const StatCard = ({ icon, label, value, desc, highlight }: any) => (
  <div className={`p-3 md:p-5 rounded-xl border flex flex-col justify-between h-28 md:h-32 transition-all hover:scale-[1.02]
    ${highlight 
      ? 'bg-blue-900/10 border-blue-500/50 shadow-[0_0_20px_rgba(59,130,246,0.15)]' 
      : 'bg-[#1E293B] border-slate-700/50'}`}>
    <div className="flex justify-between items-start">
      <div className="p-1.5 md:p-2 bg-[#0F172A] rounded-lg border border-slate-700">{icon}</div>
      <span className="text-xl md:text-2xl font-bold text-white">{value}</span>
    </div>
    <div>
      <div className={`text-xs md:text-sm font-semibold ${highlight ? 'text-blue-200' : 'text-slate-300'}`}>{label}</div>
      <div className="text-[10px] md:text-xs text-slate-500 truncate">{desc}</div>
    </div>
  </div>
);
  ...
  return (
    <div className={`p-4 md:p-5 rounded-xl bg-[#1E293B] border ${borderColor} relative group overflow-hidden`}>
      ...
      <div className="flex gap-3 md:gap-4">
        ...
        <div>
          <h4 className="font-bold text-sm md:text-base text-slate-200 group-hover:text-white transition-colors">{title}</h4>
          <p className="text-xs md:text-sm text-slate-400 mt-1 mb-2 leading-relaxed">{desc}</p>
          <span className="text-[10px] md:text-xs text-slate-600 font-medium flex items-center gap-1">
...
const Badge = ({ color, name, opacity }: any) => (
  <div className={`aspect-square rounded-xl flex flex-col items-center justify-center gap-2 p-2 
    ...
    <div className={`w-6 h-6 md:w-8 md:h-8 rounded-full ${color} shadow-lg`}></div>
    <span className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase">{name}</span>
  </div>
);
